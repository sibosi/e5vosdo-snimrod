...
import { EventType, getEventEvents } from "@/db/event";
import {GET as getEJGEvents} from "@/app/api/getEJGEvents/route";
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch combined events" }, { status: 500 });
  }
}