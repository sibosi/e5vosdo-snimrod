...
const PauseCircleIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
...
const PlayIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
  ...
  >
    <path
      d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM10 15.5V8.5L16 12L10 15.5Z"
      fill="currentColor"
    />
  </svg>
);

const NextIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
...
const PreviousIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
...
const RepeatOneIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
...
const ShuffleIcon = ({ size = 24, width = 24, height = 24, ...props }) => (
  <svg
    aria-hidden="true"
    fill="none"
    focusable="false"
...
export default function Player() {
  ...
  const [podcastNames, setPodcastNames] = React.useState<string[]>();
  const [currentPodcast, setCurrentPodcast] =
    React.useState<string>("radu.mp3");
  const [isPlaying, setIsPlaying] = React.useState(false);
  const [podcastImage, setPodcastImage] = React.useState<string>(
    "/groups/kektura.jpg",
  );

  const [podcastImages, setPodcastImages] = React.useState<{
    [key: string]: string;
  }>();
  ...
  useEffect(() => {
    setPodcastImage(podcastImages?.[currentPodcast] ?? "/groups/podcast.png");
  }, [currentPodcast, podcastImages]);

  useEffect(() => {
    extractColors(podcastImage).then((colors) => {
      setBgColor(colors[0].hex);
      console.log(colors);
      setTextColor(colors[1].hex);
    });
  }, [podcastImage]);
  ...
  const timeToDisplay = (time: number) => {
  ...
  if (!podcastNames) return <p>Loading...</p>;

  if (podcastNames.length === 0) return <p>Nincs elérhető podcast</p>;

  return (
    <Card
      isBlurred
      className="max-w-[610px] border-none"
      style={{
          ...
          <div className="col-span-6 flex flex-col md:col-span-8">
            <div className="flex items-start justify-between">
              <div className="flex flex-col gap-0">
                <p className="flex gap-1 text-small text-foreground/80">
                  <h3 className="font-semibold text-foreground/90">
                    E5 Podcast |
                  </h3>
                  {podcastNames.map((podcast, index) => (
                    <>
                      {index !== 0 && (
                        <span key={podcast} className="text-foreground/60">
                          •
                        </span>
                      )}
                      <button
                        key={podcast}
                        className="text-selfprimary/60"
                ...
                <h1 className="mt-2 text-large font-medium">
                  {currentPodcast.split(".")[0]}
                </h1>
              </div>
              <Button
                isIconOnly
                className="hidden -translate-y-2 translate-x-2 text-default-900/60 data-[hover]:bg-foreground/10"
                radius="full"
                variant="light"
            ...
            <div className="mt-3 flex flex-col gap-1">
              ...
              <div className="flex justify-between">
                <p className="text-small">{timeToDisplay(currentTime)}</p>
                <p className="text-small text-foreground/50">
                  {timeToDisplay(audioRef.current?.duration ?? 0)}
                </p>
              </div>
            </div>
            ...
            <div className="flex w-full items-center justify-center">
              ...
              <Button
                ...
                onPress={() => {
                  setCurrentPodcast(
                    podcastNames[
                      (podcastNames.indexOf(currentPodcast) + 1) %
                        podcastNames.length
                    ],
                  );
                  pausePodcast();
              ...
              <Button
                isIconOnly
                className="hidden data-[hover]:bg-foreground/10"
                radius="full"
                variant="light"
              >
            ...
            <audio
              ref={audioRef}
              src={`/api/podcast/${currentPodcast}`}
              controls
              className="hidden"
            >
