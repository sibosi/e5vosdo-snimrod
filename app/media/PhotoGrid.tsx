...
const MasonryGrid: React.FC<MasonryGridProps> = ({
  ...
  const [expectedImageHeights, setExpectedImageHeights] = React.useState<number[]>([]);
  ...
  React.useEffect(() => {
    ...
    const handleResize = () => {
      ...
      setColHeight(Array(newColumns).fill(0));
      setImagesInColumn(Array.from({ length: newColumns }, () => []));
      setImageIndex(0); // Reset to recalculate layout
    };
  ...
  }, [photos, expectedImageWidth]);

  React.useEffect(() => {
    if (!isColLoaded || photos.length === 0) return;
    
    // Calculate all image placements at once
  ...
  return (
    <div id="gridParent" className='grid' style={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, 1fr)`, gap }}>
      {imagesInColumn.map((column, colIndex) => (
        <div key={colIndex} style={{ display: 'flex', flexDirection: 'column', gap }}>
          {column.map(image => (
            <div key={`ImageWrapper${image.id}`} style={{ minWidth: expectedImageWidth, minHeight: expectedImageHeights[image.id - 1] }}>
              <Image priority={false} src={image.url} alt={image.title} width={expectedImageWidth} height={expectedImageHeights[image.id - 1]} className='bg-selfprimary-500'/>
            </div>
          ))}
        </div>
      ))}
