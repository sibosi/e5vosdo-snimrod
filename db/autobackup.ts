...
async function backupAndUpload() {
  ...
  try {
    ...
    const dumpCommand = `/usr/bin/mysqldump -h ${process.env.MYSQL_HOST} -u ${process.env.MYSQL_USER} -p${process.env.MYSQL_PASSWORD} ${process.env.MYSQL_DATABASE} > ${dumpFilePath}`;
