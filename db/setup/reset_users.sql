...
CREATE TABLE
    IF NOT EXISTS `users` (
        ...
        `analytics_id` VARCHAR(36) UNIQUE DEFAULT NULL
...
SET
    NEW.coming_year = SUBSTRING(NEW.EJG_code, 1, 4);

SET
    NEW.class_character = SUBSTRING(NEW.EJG_code, 5, 1);

SET
    NEW.order_number = SUBSTRING(NEW.EJG_code, 6, 2);

SET
    NEW.analytics_id = UUID ();
